...
for the new version and tags it. The tag builds the image, runs the smoke test
`tests/smoke-image.sh` on it and pushes exactly that image, then creates the
GitHub release with the notes of that section.
